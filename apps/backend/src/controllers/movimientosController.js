...
// Usa lógica FIFO definida en la función SQL fn_registrar_salida
async function registrarSalida(req, res, next) {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT fn_registrar_salida($1,$2,$3,$4,$5) AS movimiento_id`,
    ...
    // fn_registrar_salida lanza EXCEPTION si no hay stock suficiente
...
async function getDashboardStats(req, res, next) {
  try {
    const [total, critico, alertasVenc, movHoy] = await Promise.all([
...
module.exports = { registrarEntrada, registrarSalida, getMovimientos, getAlertas, getDashboardStats }
