            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-surface-container-low border-b text-xs uppercase font-bold text-zinc-500">
                  ...
                  <th className="px-5 py-5">Categoría</th>
                  <th className="px-5 py-5">Stock</th>
                  <th className="px-5 py-5">Precio Normal</th>
                  <th className="px-5 py-5">V.DESC</th>
                  <th className="px-5 py-5">Valor Total</th>
              ...
              <tbody className="divide-y divide-zinc-100">
                {loading ? (
                  <tr><td colSpan={9} className="px-5 py-10 text-center text-on-surface-variant">Cargando...</td></tr>
                ) : filtrados.length === 0 ? (
                  <tr><td colSpan={9} className="px-5 py-10 text-center text-on-surface-variant">Sin resultados</td></tr>
                ) : filtrados.map(p => (
                  <tr key={p.id} className="hover:bg-zinc-50 transition-colors">
                    <td className="px-5 py-4">
                      <p className="font-mono text-xs font-bold">{p.sku}</p>
                      {p.codigo_barras && <p className="font-mono text-[10px] text-zinc-400 mt-0.5">{p.codigo_barras}</p>}
                    </td>
              ...
              {!loading && filtrados.length > 0 && (
                <tfoot>
                  <tr className="bg-zinc-50 border-t-2 border-zinc-200">
                    <td colSpan={6} className="px-5 py-4 text-right font-black text-sm text-on-surface-variant uppercase tracking-wider">
                      {busqueda ? `Total filtrado (${filtrados.length})` : `Gran Total (${productos.length} productos)`}
                    </td>
                    <td className="px-5 py-4 font-black text-xl text-primary">
